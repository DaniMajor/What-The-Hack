# USAGE
# python MusicTherapy_w2.py --ip 127.0.0.1 --port 5000
import os
...
def check_mood():
	...
	if mood == "Error":
		...
		# print(comments)
		feedback = 'Your mood is ' + str(mood)
	user_data = feedback, songName, songUrl, comments
	return render_template("index.html", user_image=fakeUrl, user_data=user_data)
...
def detect_motion(frameCount):
	...
	while True:
		# settings.logging.debug("looping over frames of video stream")
		# read the next frame from the video stream, resize it,
		# convert the frame to grayscale, and blur it
		frame = vs.read()
		frame = imutils.resize(frame, width=600)
		# settings.logging.debug("frame successfully resized and ready for analysis")
		# gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
		# gray = cv2.GaussianBlur(gray, (7, 7), 0)

		# grab the current timestamp and draw it on the frame
		timestamp = datetime.datetime.now()
		# settings.logging.debug("Photo was taken at " + str(timestamp))
		...
		with lock:
			...
			# settings.logging.debug("lock acquired, output frame set, lock released")
    ...
    for moodrec in data:
        if moodrec['mood'] == in_mood:      # search for the mood
            ...
            for songrec in moodrec["songs"]:    # loop through all the song selections for the mood
                # settings.logging.debug("recommended song to user based off of mood")
                # print("Mood:", moodrec['mood'], ", Index:", index, " ", songrec)
                index += 1;
...
if __name__ == '__main__':
	...
	# settings.logging.debug('successfully construct the argument parser and parse command line arguments')

# release the video stream pointer
vs.stop()
# settings.logger.debug("successfully released video stream pointer")